...
void scan_callback(const sensor_msgs::PointCloud2 cloud_msg)
{
    ...
    box_filter_.setMax(Eigen::Vector4f(2.0, 1.0, 2.0, 1.0));  //3.0, 1.0, 3.0, 1.0
    box_filter_.setMin(Eigen::Vector4f(-2.0, -1.2, 0.0, 1.0));  //-3.0, -1.2, 0, 1.0
...
{
    ros::init(argc, argv, "scan_nose_filter_node");
    ros::NodeHandle nh;
    // ROS related
    pub_combined_image = nh.advertise<sensor_msgs::PointCloud2>("/scan_nose_pointcloud_filtered", 1);
