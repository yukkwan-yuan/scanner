...
{
    ros::init(argc, argv, "scan_person_front_filter_node");
    ros::NodeHandle nh;
    // ROS related
    pub_combined_image = nh.advertise<sensor_msgs::PointCloud2>("/scan_person_front_pointcloud_filtered", 1);
    ros::Subscriber scan_sub = nh.subscribe("/camera3/depth_registered/points", 1, scan_callback);
