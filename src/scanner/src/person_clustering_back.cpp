...
ScanImageCombineNode::ScanImageCombineNode(ros::NodeHandle nh, ros::NodeHandle pnh): nh_(nh), pnh_(pnh)
{
    ...
    ros::param::param<string>("~scan_topic", scan_topic, "/scan_person_back_pointcloud_filtered");
    ros::param::param<string>("~img_topic", img_topic, "/camera2/color/image_raw"); 
    ros::param::param<string>("~caminfo_topic", caminfo_topic, "/camera2/color/camera_info"); 

    // ROS publisher & subscriber & message filter
    pub_combined_image_ = nh.advertise<sensor_msgs::Image>("/scan_person_clustering_back_node/debug_reprojection", 1);
    pub_marker_array_ = nh.advertise<visualization_msgs::MarkerArray>("/scan_person_clustering_back_node/obj_marker", 1);
    pub_colored_pc_ = nh.advertise<sensor_msgs::PointCloud2>("/scan_person_clustering_back_node/colored_pc", 1);
    pub_detection3d_ = nh.advertise<detection_msgs::Det3DArray>("/scan_person_clustering_back_node/det3d_result", 1);
...
{
    ros::init(argc, argv, "scan_person_clustering_back_node");
