...
if [ $# -gt 0 ]; then
    if [ "$1" == "same" ]; then
        docker exec -it mars-container bash
    else
        ${DOCKER_CMD} run ${NVIDIA_SUPPORT_OPTION} --name mars-container --rm -it --net=host --privileged -v /dev:/dev \
            -e DISPLAY=$DISPLAY \
            ...
            alvis1029/mars-docker:${DOCKER_TAG}      
    fi
else
